...
describe("content", () => {
  it("has four products, six cases, fourteen verify rows", () => {
    ...
    expect(en.enterprise.cases).toHaveLength(6);
  ...
  it("references only images that exist in public/img", () => {
    const imgs = [
      ...
      ...en.enterprise.cases.map(c => c.shot).filter((s): s is string => Boolean(s)),
  ...
  it("every case has exactly one of shot or schematic", () => {
    for (const c of en.enterprise.cases) expect(Boolean(c.shot) !== Boolean(c.schematic), c.key).toBe(true);
  });
  ...
  it("keeps Arabic strings Arabic (no reversed-Latin fakes) for the hero and section titles", () => {
    for (const s of [ar.hero.name, ar.hero.titleA, ar.hero.line, ar.productsSection.title, ar.enterprise.title, ar.career.title, ar.verify.title]) {
